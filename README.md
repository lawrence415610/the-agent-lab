...
💡 Experiment 1
这里有一个简单伪·MCP实现：要求LLM输出一个json格式的数据，再用python来根据这个json调用工具：
...
## Day 3
LLM是一个基于输入来产生一堆token输出的语言模型，但是Agent则需要能够独立地理解并且执行。
如何做到呢？Agent的实现变得更像是玩弄一场语言游戏。比如：
1. 如何实现智能体的记忆？
答：每次把以前讲过的东西都重新发给LLM，让其基于前面的上下文继续生成，看起来就像是记住了一样。
2. 现在的ChatGPT产品里面有推理能力强度的选择，这是怎么做到的呢？
答：简单来说，就是在提示词上下功夫，比如说，你告诉模型先仔细思考再作答，要求模型把思考过程写出来，这样逼迫它生成一个具有推理内容的答案，这样错误答案被脱口而出的可能性就变小了。这类似于让一个人谨慎作答，他想得就更多。当他的逻辑被摆出来之后，答案也必须与前面的逻辑有一致性，如此一来模型回答得就更加准确。
